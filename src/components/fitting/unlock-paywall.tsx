...
export function UnlockPaywall({ sessionId, isSignedIn }: Props) {
  ...
  useEffect(() => {
    if (!polling) return;
    const guestToken = getGuestToken(sessionId);
    let attempts = 0;
  ...
  async function handleUnlock() {
    ...
    if (data.alreadyUnlocked) { router.refresh(); return; }
      ...
      <div className="relative">
        ...
        <div className="absolute inset-0 flex items-start justify-center pt-6">
          <div className="bg-white rounded-2xl border border-gray-200 shadow-xl p-8 w-full max-w-lg mx-4">
            ...
            <Button variant="gold" size="lg" className="w-full" onClick={handleUnlock} disabled={loading}>
