...
export async function POST(req: NextRequest) {
  ...
  if (type === "consumer_unlock") {
    ...
    // Already unlocked — no need to charge
  ...
  const checkout = await stripe.checkout.sessions.create({
    ...
    metadata: { retailerId: retailer.id, plan: (plan ?? "STARTER").toUpperCase() },
    subscription_data: {
      metadata: { retailerId: retailer.id, plan: (plan ?? "STARTER").toUpperCase(), fittingsLimit: String(PLAN_LIMITS[(plan ?? "STARTER").toUpperCase()] ?? 50) },
    },
  });
